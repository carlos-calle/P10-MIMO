...
- Deteccion MIMO por subportadora con ZF, MMSE o MMSE-SIC.
- Comparacion multiantena visual y BER con bits aleatorios entre escenarios `2x2`, `4x2` y `4x4`, usando `MMSE` y `SIC`.
- Selector de capas espaciales para comparar `2 capas` o `maximo de capas`.
...
4. Selecciona `2 capas` para una comparacion BER mas justa o `Maximo de capas` para mostrar la extension 4x4 completa.
5. Usa "PRUEBA MULTIANTENA" o "CURVAS BER".

La pestana "Imagen Original" muestra solo la imagen seleccionada como referencia. `PRUEBA MULTIANTENA` usa esa imagen para generar una grilla visual de seis reconstrucciones con BER. `CURVAS BER` usa bits aleatorios para comparar `2x2`, `4x2` y `4x4` con `MMSE` y `SIC`. En `2 capas`, tambien el caso `4x4` transmite dos capas; en `Maximo de capas`, `4x4` transmite cuatro capas.
...
13. Se ecualiza SISO o se detectan las capas MIMO con ZF, MMSE o MMSE-SIC.
14. Se demapean simbolos a bits.
15. Se revierte el scrambling.
16. Se calcula BER y se reconstruye la imagen recibida.

Para la curva BER Monte Carlo se generan bits aleatorios reproducibles como carga util. Esto permite comparar modulaciones y arreglos MIMO sin depender del contenido de una imagen particular. La UI muestra los parametros LTE principales, usando por defecto `10 MHz`, CP normal y un camino multipath. La comparacion multiantena repite la simulacion para 2x2, 4x2 y 4x4 con detectores MMSE y SIC usando la modulacion seleccionada.
...
- `core/mimo_ops.py`: precoding, canal efectivo `H_eff`, detectores ZF/MMSE/SIC y cancelacion sucesiva.
...
- La interfaz muestra BW, CP y multipath para dejar visibles los parametros LTE de base; por defecto usa `10 MHz`, CP normal y un camino multipath.
...
- Detectores lineales ZF y MMSE por subportadora, alineados con la explicacion de recuperacion mediante `W = H^-1` y su variante regularizada frente a ruido.
...
- Comparacion visual multiantena sobre la imagen cargada y BER multiantena sobre bits aleatorios para 2x2/4x2/4x4 con MMSE y SIC.
